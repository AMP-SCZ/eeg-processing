...
% SCN: this has major indexing problems, as do other FASTER programs?  don't use any eeg_chans inputs
%      that don't conform to 1:1:#!
...
%     corrected = mcorrs(dist_inds) - fitcurve(idist_inds);
    corrected = mcorrs - fitcurve(idist_inds);					% edited SCN 9/13/2021

    list_properties(:,measure) = corrected;
else
%     list_properties(:,measure) = mcorrs(dist_inds);
    list_properties(:,measure) = mcorrs;						% edited SCN 9/13/2021
end
measure = measure + 1;
