...
%{
		% targeteted segmenting, don't dump so much stuff to command window
		foundZip  = AMPSCZ_EEG_findZips;
		foundBIDS = AMPSCZ_EEG_findProcSessions;
		kAttempt  = ~ismember( strcat( foundZip(:,2), foundZip(:,3) ), strcat( foundBIDS(:,2), foundBIDS(:,3) ) );
		clear foundBIDS
		foundZip(~kAttempt,:) = [];		% { networkSite, subjId, date acquired, file date }
		for iZip = 1:size( foundZip, 1 )
			siteGroup = foundZip{iZip,1}(1:end-2);
			siteName  = foundZip{iZip,1}(end-1:end);
			
			iSite = find( strcmp( siteName, siteInfo(:,1) ) & strcmp( siteGroup, siteInfo(:,2) ) );
			...
			zipFiles = dir( fullfile( rawDir, foundZip{iZip,2}, 'eeg', '*.zip' ) );		% could use subjDirs(isubj).folder
			...
			if numel( zipFiles ) == 0
				continue		% there has to be zip files!
			else
					...
					if char1 == abs( '0' )		% this is always true until something more sophisticated is done later
						...
						for iZip = 1:nZip
							...
							zipContent = listZipContents( zipFile );		% #x1 cell array, relative to zip file parent directory
								...
								[ ~, zipName ] = fileparts( zipContent );
								zipContent(cellfun( @isempty, zipName )) = [];
								% ignore Mac-injected junk
								zipContent(~cellfun( @isempty, strfind( zipContent, 'DS_Store' ) )) = [];
								zipContent(strncmp( zipContent, '__MACOS', 7 )) = [];								
							% -- check for existence of unzipped files
							unzipCheck = cellfun( @(u)exist(u,'file')==2, fullfile( outputDir, zipContent ) );
							[ zipContentPath, zipContentFile, zipContentExt ] = fileparts( zipContent );	% extensions include initial '.'
							if all( unzipCheck )
